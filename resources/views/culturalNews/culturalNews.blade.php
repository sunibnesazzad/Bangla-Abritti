			            ...
			            <div class="panel cult_evnt panel-danger">
			                <div class="news_img">
								@if($culturalNew->IMAGE_FILE_PATH != NULL)
									<img src="{{asset('bangla_abritti/uploaded_img/cultural_news_images/'.$culturalNew->IMAGE_FILE_PATH)}}">
								@else
									<img src="{{asset('bangla_abritti/uploaded_img/cultural_news_images/occ.jpg')}}">
								@endif
			                </div>
			                <div class="news_title_dis">
			                    <h3>{{ $culturalNew->NEWS_TITLE }}</h3> 
			                    <hr>
			                    <p>{!! str_limit($culturalNew->NEWS_DESC,130) !!}</p>
			                </div>
			                <a href="/culturalNews/show/{{ $culturalNew->CULTURAL_NEWS_ID }}" type="button" class="btn btn-success read-more">বিস্তারিত</a>
			            </div>
			        </div>
			    @endforeach     
